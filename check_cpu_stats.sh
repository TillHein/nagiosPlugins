...
#Changelog
#Till Hein | 10.12.2018 | removed limitation (-n 5)of top command for darwin OS  
#
...
case `uname` in
	Linux ) CPU_REPORT=`iostat -c 5 2 |  tr -s ' ' ';' | sed '/^$/d' | tail -1`
 	...
 	AIX )   CPU_REPORT=`iostat -t 5 2 | sed -e 's/,/./g'|tr -s ' ' ';' | tail -1`
            ...
            ;;
    Darwin ) CPU_REPORT=`/usr/bin/top -l 1|grep 'CPU usage'|sed -e 's/%//g'`
            CPU_USER=`echo $CPU_REPORT | cut -d " " -f 3`
            CPU_SYSTEM=`echo $CPU_REPORT | cut -d " " -f 5`
            CPU_IOWAIT=0
            CPU_IDLE=`echo $CPU_REPORT | cut -d " " -f 7`
            ;;
...
#	exit $STATE_OK
	...
	if [[ ${CPU_IOWAIT} > $WARNING_THRESHOLD ]] && [[ ${CPU_IOWAIT} < ${CRITICAL_THRESHOLD} ]]; then
	    label="CPU IOWAIT WARNING:"
	    result=$STATE_WARNING
	elif [[ ${CPU_IOWAIT} > $CRITICAL_THRESHOLD ]]; then
	...
	CPU_USER2=${CPU_USER}
	if [[ ${CPU_USER} < 1.0 ]]; then
	    CPU_USER2=0.0
	fi
	CPU_SYSTEM2=${CPU_SYSTEM}
	if [[ ${CPU_SYSTEM} < 1.0 ]]; then
	    CPU_SYSTEM2=0.0
	fi
	CPU_IOWAIT2=${CPU_IOWAIT}
	if [[ ${CPU_IOWAIT} < 1.0 ]]; then
	    CPU_IOWAIT2=0.0
	fi
	CPU_IDLE2=${CPU_IDLE}
	if [[ ${CPU_IDLE} < 1.0 ]]; then
	    CPU_IDLE2=0.0
	fi
